...
public class DifficultyState extends State{
	...
	protected void handleInput() {
		if(Gdx.input.isKeyJustPressed(Keys.S) || Gdx.input.isKeyJustPressed(Keys.UP)){
			currentChoice++;
			if(currentChoice > options.length-1){
				currentChoice = 0;
			}
		}
		if(Gdx.input.isKeyJustPressed(Keys.W) || Gdx.input.isKeyJustPressed(Keys.DOWN)){
			currentChoice--;
			if(currentChoice < 0){
				currentChoice = options.length - 1;
			}
	...
	public void render() {
		...
		sb.draw(blank, WIDTH/2.3f - 30, HEIGHT/2.3f - 20);
