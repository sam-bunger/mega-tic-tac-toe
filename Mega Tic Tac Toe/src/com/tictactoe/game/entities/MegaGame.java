...
public class MegaGame {
	...
	public MegaGame(GameStateManager gsm) {
	...
	public void update(float delta){
		...
		if(Gdx.input.justTouched()){
			...
			if(a.length != 1){
				int x;
				do{
					x = game[a[0]][a[1]].addXO(a[3], a[2], currentTurn);
				}while(x == 3);
				
				if(x == 0){
					switchTurn();
					int num = checkWinner(a[0], a[1], 0);
					if(num == 0){
						winner = 0;
					}else if(num == 2){
						winner = 2;
				...
				if(game[a[2]][a[3]].getFinished() == -1){
					for(int i = 0; i < size; i++){
						for(int j = 0; j < size; j++){
							game[i][j].setActivation(false);
						}
					}
					game[a[2]][a[3]].setActivation(true);
				}else{
					for(int i = 0; i < size; i++){
						for(int j = 0; j < size; j++){
